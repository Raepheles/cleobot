...
public class AboutCommand {
    ...
    public static void aboutCommand(CommandContext command) {
        ...
        String owner = command.getClient().getUserByID(Utilities.getOwnerId()).getName();
        String ownerAvatar = command.getClient().getUserByID(Utilities.getOwnerId()).getAvatarURL();
        String botAvatar = command.getClient().getOurUser().getAvatarURL();
        String ownerTag = command.getClient().getUserByID(Utilities.getOwnerId()).getDiscriminator();
        ...
        String desc =  "**" + command.getClient().getOurUser().getName() + "** is a bot I made for mobile game called **King's Raid**. " +
                ...
                "You can contact me via `feedback` command. If you have servers in common with me like " +
                "reddit community server you can also DM me. I also have private server where I test my stuff if you want you can also " +
                "join there by clicking my name above.";
        embed.withDesc(desc);
